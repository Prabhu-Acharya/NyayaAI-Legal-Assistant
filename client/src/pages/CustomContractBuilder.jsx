/**
 * client/src/pages/CustomContractBuilder.jsx
 * Week 9 — Co-founder / T&C / Privacy / Legal Notice builder
 * + Polish-clause AI + E-sign before PDF download
 *
 * Route: /dashboard/custom-contracts
 * Add to DashboardLayout sidebar and App.jsx routes.
 */
...
// ── Styles (matches existing ContractGenerator palette) ──────────────────────
const S = {
  ...
  typeCard:    (active) => ({ padding: '20px 16px', borderRadius: '12px', cursor: 'pointer', textAlign: 'center', border: active ? '2px solid #c9a84c' : '1px solid rgba(201,168,76,0.2)', background: active ? 'rgba(201,168,76,0.12)' : 'rgba(255,255,255,0.03)', transition: 'all 0.2s' }),
  typeIcon:    { fontSize: '28px', display: 'block', marginBottom: '8px' },
  typeLabel:   (active) => ({ fontSize: '13px', fontFamily: 'sans-serif', color: active ? '#c9a84c' : '#a0917a', fontWeight: active ? '600' : '400' }),
  ...
  btnSm:       { padding: '8px 18px', background: 'transparent', color: '#c9a84c', border: '1px solid rgba(201,168,76,0.4)', borderRadius: '6px', fontSize: '13px', cursor: 'pointer', fontFamily: 'sans-serif' },
  ...
  success:     { color: '#4ade80', fontSize: '13px', fontFamily: 'sans-serif', marginBottom: '12px' },
...
export default function CustomContractBuilder() {
  ...
  const handleGenerate = async () => {
    ...
    setLoading(true);
    try {
      const { data } = await API.post('/api/contracts/generate-custom', {
        type: selectedType, formData, language,
      });
  ...
  const handlePolish = async () => {
    ...
    setPolished('');
    try {
      const { data } = await API.post('/api/contracts/polish-clause', {
        clauseText:  polishText,
        instruction: polishInstr || undefined,
      });
      setPolished(data.improved);
    } catch (err) {
      setPolished('❌ ' + (err.response?.data?.message || 'Polish failed'));
    } finally {
      setPolishing(false);
    }
  };
  ...
  const handleDownloadTxt = () => {
    ...
    a.href = url; a.download = `${cfg.label.replace(/\s+/g,'_')}.txt`; a.click();
      ...
      <div style={S.container}>
        ...
        {step === 1 && (
          <>
            ...
            <button style={S.btn} disabled={!selectedType} onClick={() => setStep(2)}>
              Continue →
            </button>
          </>
        )}
        ...
        {step === 2 && cfg && (
          <>
            <div style={S.row}>
              <span style={{ color: '#c9a84c', fontFamily: 'sans-serif', fontSize: '18px' }}>
                {cfg.icon} {cfg.label}
              </span>
        ...
        {step === 3 && (
          <>
            <div style={S.row}>
              <span style={{ color: '#c9a84c', fontFamily: 'sans-serif', fontSize: '18px' }}>
                {cfg?.icon} {cfg?.label} — Preview
              </span>
              <button style={S.btnSm} onClick={() => setStep(2)}>← Edit</button>
              <button style={S.btnSm} onClick={handleDownloadTxt}>⬇ Download .txt</button>
              {contractId && (
                <a
                  style={{ ...S.btnSm, textDecoration: 'none', display: 'inline-block' }}
                  href={`${import.meta.env.VITE_API_URL || 'https://nyayaai-backend-qvih.onrender.com'}/api/contracts/${contractId}/export/pdf?token=${localStorage.getItem('token')}`}
            ...
            {/* E-sign */}
            <h3 style={{ color: '#c9a84c', fontFamily: 'sans-serif', fontSize: '16px', marginBottom: '12px' }}>
              ✍️ E-Signature
            </h3>
            <ESignature
              onSign={(url) => setSigUrl(url)}
              onClear={() => setSigUrl(null)}
            />
            {sigUrl && (
              <div style={{ marginTop: '12px', fontFamily: 'sans-serif', fontSize: '13px', color: '#a0917a' }}>
                <p style={{ marginBottom: '6px' }}>Signature preview:</p>
                <img src={sigUrl} alt="signature" style={{ background: '#fff', borderRadius: '6px', padding: '4px', maxWidth: '260px' }} />
              </div>
            )}

            {/* Polish Clause */}
            <h3 style={{ color: '#c9a84c', fontFamily: 'sans-serif', fontSize: '16px', margin: '28px 0 12px' }}>
              🪄 Polish a Clause
            </h3>
            <div style={S.clauseBox}>
              ...
              <textarea
                ...
                placeholder="Paste any clause from the contract above…"
              />
              ...
              <input
                ...
                placeholder="e.g. Make more enforceable, simplify language…"
              />
